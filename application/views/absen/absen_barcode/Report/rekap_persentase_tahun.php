                            ...
                            foreach ($dept->result() as $dp) {
                              ...
                              foreach ($bagian->result() as $b) {
                                ...
                                if($karyawan->num_rows() == 0)
                                {
                                  ...
                                  foreach ($karyawan->result() as $k) {
                                    $hitung_hk = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan join hris.bagian b on b.recid_bag = k.recid_bag where b.recid_bag = $b->recid_bag and k.recid_karyawan = $k->recid_karyawan and year(h.tanggal) = $tahun and (status != 24 and status != '28' and status != '31' and status != '32' and status != '33' and status != '36' and is_closed = '0'); ");
                                    ...
                                    $m = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun  and h.status = '11' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $p1 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '5' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $sid = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '2' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $kk = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '25' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $h1 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '6' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $h2 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '7' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $p4 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '12' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $c = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '3' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                     $mu = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and h.status = '34' and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    $fnorma = $db2->query("SELECT count(h.recid_absen) as jml_norma FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and $norma and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    foreach ($fnorma->result() as $fnorma) {
                                      $jml_norma = $fnorma->jml_norma;
                                    } 

                                    $fnnorma = $db2->query("SELECT count(h.recid_absen) as jml_nonnorma FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where year(tanggal) = $tahun and $non_norma and k.recid_karyawan = $k->recid_karyawan  and is_closed = '0'");

                                    foreach ($fnnorma->result() as $fnnorma) {
                                      $jml_nonnorma = $fnnorma->jml_nonnorma;
                                    } 

                                    $telat = $db2->query("SELECT * from izin where year(tgl_izin) = $tahun and (jenis = 'Terlambat' or jenis = 'Terlambat Terencana' or jenis = 'Terlambat Tidak Terencana')  and recid_karyawan = $k->recid_karyawan and is_delete = '0'");
                                    ...
                                    $ringan = $db2->query("SELECT * from absensi_hris where year(tanggal) = $tahun and jenis_absen = 2 and kategori = 'Ringan' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");

                                    $lanjut = $db2->query("SELECT * from absensi_hris where year(tanggal) = $tahun and jenis_absen = 2 and kategori = 'Berkelanjutan' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");

                                    $berat =  $db2->query("SELECT * from absensi_hris where year(tanggal) = $tahun and jenis_absen = 2 and kategori = 'Berat' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");
                                ...
                                if($kar_bagian != 0)
                                  ...
                                  $d_target_tk = (($d_hk - $d_nonnorma)/$d_hk2) * 100;
                                  $d_m = $d_m + $s_m;
                                  $d_p1 = $d_p1 + $s_p1;
                                  $d_sid = $d_sid + $s_sid;
                                  $d_kk = $d_kk + $s_kk;
                              ...
                              <?php } ?><!--  end forach bagian -->
                                 ...
                                 <?php  /*------------ department calculation start ----------------*/
                                  ...
                                  $dg_nonnorma_p = ($dg_nonnorma / $dg_hk2) * 100;
                                  $dg_target_tk = (($dg_hk - $dg_nonnorma)/$dg_hk2) * 100;
                                  $dg_m = $dg_m + $d_m;
                                  $dg_p1 = $dg_p1 + $d_p1;
                                  $dg_sid = $dg_sid + $d_sid;
                                  $dg_kk = $dg_kk + $d_kk;
                                  ...
                                  $c_target_tk = (($c_hk - $c_nonnorma)/$c_hk2) * 100;
                                  $c_m = $c_m + $d_m;
                                  $c_p1 = $c_p1 + $d_p1;
                                  $c_sid = $c_sid + $d_sid;
                                  $c_kk = $c_kk + $d_kk;

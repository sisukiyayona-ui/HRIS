    ...
    <div class="">
      <div class="page-title">
        <div class="title_left">
          <h3>Rekapitulasi Persentase Kehadiran Mingguan Karyawan</h3>
        </div>
      </div>
              ...
              <table class="table table-bordered" id="rekap_absen" border="1">
                ...
                <tbody>
                      ...
                      foreach ($bagian->result() as $b) {
                        ...
                        if ($karyawan->num_rows() == 0) {
                          ...
                          foreach ($karyawan->result() as $k) {
                            $hitung_hk = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan join hris.bagian b on b.recid_bag = k.recid_bag where k.recid_karyawan = $k->recid_karyawan and (tanggal between '$mulai' and '$sampai') and (status != 24 and status != '28' and status != '31' and status != '32' and status != '33' and status != '36');");
                            ...
                            $m = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai')  and h.status = '11' and k.recid_karyawan = $k->recid_karyawan");

                            $p1 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '5' and k.recid_karyawan = $k->recid_karyawan");

                            $sid = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '2' and k.recid_karyawan = $k->recid_karyawan");

                            $kk = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '25' and k.recid_karyawan = $k->recid_karyawan");

                            $h1 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '6' and k.recid_karyawan = $k->recid_karyawan");

                            $h2 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '7' and k.recid_karyawan = $k->recid_karyawan");

                            $p4 = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '12' and k.recid_karyawan = $k->recid_karyawan");

                            $c = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '3' and k.recid_karyawan = $k->recid_karyawan");

                             $mu = $db2->query("SELECT * FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and h.status = '34' and k.recid_karyawan = $k->recid_karyawan");


                            $fnorma = $db2->query("SELECT count(h.recid_absen) as jml_norma FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and $norma and k.recid_karyawan = $k->recid_karyawan");

                            foreach ($fnorma->result() as $fnorma) {
                              $jml_norma = $fnorma->jml_norma;
                            }
                            // echo $norma;
                            $fnnorma = $db2->query("SELECT count(h.recid_absen) as jml_nonnorma FROM hadir_barcode h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (tanggal between '$mulai' and '$sampai') and $non_norma and k.recid_karyawan = $k->recid_karyawan");

                            foreach ($fnnorma->result() as $fnnorma) {
                              $jml_nonnorma = $fnnorma->jml_nonnorma;
                            }

                            $telat = $db2->query("SELECT * from izin where (tgl_izin between '$mulai' and '$sampai') and (jenis = 'Terlambat' or jenis = 'Terlambat Terencana' or jenis = 'Terlambat Tidak Terencana')  and recid_karyawan = $k->recid_karyawan and is_delete = '0'");

                            $pulang = $db2->query("SELECT * from izin where (tgl_izin between '$mulai' and '$sampai') and jenis = 'Pulang' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");
                            ...
                            $ringan = $db2->query("SELECT * from absensi_hris where (tanggal between '$mulai' and '$sampai') and jenis_absen = 2 and kategori = 'Ringan' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");

                            $lanjut = $db2->query("SELECT * from absensi_hris where (tanggal between '$mulai' and '$sampai') and jenis_absen = 2 and kategori = 'Berkelanjutan' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");

                            $berat = $db2->query("SELECT * from absensi_hris where (tanggal between '$mulai' and '$sampai') and jenis_absen = 2 and kategori = 'Berat' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");
                            ...
                            $target_tk = (($jml_hk - $jml_nonnorma) / $default_hk) * 100;
                            ...
                            $s_target_tk = (($s_hk2 - $s_nonnorma) / $s_hk2) * 100;
                            $s_m = $s_m + ($m->num_rows());
                  ...
                  ?>
                      ...
                      $dg_efektif_p = ($dg_efektif / $d_hk2) * 100;
                      $dg_tdk_kerja = $dg_tdk_kerja + $d_tdk_kerja;
                      $dg_tdk_kerja_p = ($dg_tdk_kerja / $d_hk2) * 100;
                      $dg_norma = $dg_norma + $d_norma;
                      $dg_nonnorma = $dg_nonnorma + $d_nonnorma;
                      $dg_norma_p = ($dg_norma / $d_hk2) * 100;
                      $dg_nonnorma_p = ($dg_nonnorma / $d_hk2) * 100;
                      $dg_target_tk = (($d_hk2 - $dg_nonnorma) / $d_hk2) * 100;
                      $dg_m = $dg_m + $d_m;
                      $dg_p1 = $dg_p1 + $d_p1;
                      $dg_sid = $dg_sid + $d_sid;
                      $dg_kk = $dg_kk + $d_kk;
                      ...
                      $c_efektif_p = ($c_efektif / $d_hk2) * 100;
                      $c_tdk_kerja = $c_tdk_kerja + $d_tdk_kerja;
                      $c_tdk_kerja_p = ($c_tdk_kerja / $d_hk2) * 100;
                      $c_norma = $c_norma + $d_norma;
                      $c_nonnorma = $c_nonnorma + $d_nonnorma;
                      $c_norma_p = ($c_norma / $d_hk2) * 100;
                      $c_nonnorma_p = ($c_nonnorma / $d_hk2) * 100;
                      $c_target_tk = (($d_hk2 - $c_nonnorma) / $d_hk2) * 100;
                      $c_m = $c_m + $d_m;
                      $c_p1 = $c_p1 + $d_p1;
                      $c_sid = $c_sid + $d_sid;
                      $c_kk = $c_kk + $d_kk;
                    ...
                    <tr>
                      ...
                      <td></td>
                      <td></td>
                      <td><?php echo round((($dg_m / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_p1 / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_sid / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_kk / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_h1 / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_h2 / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_p4 / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_c / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_mu / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_telat / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_pulang / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_keluar / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_ringan / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_lanjut / $d_hk2) * 100), 2) ?> %</td>
                      <td><?php echo round((($dg_berat / $d_hk2) * 100), 2) ?> %</td>
                    </tr>
                  ...
                  <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td><?php echo round((($c_m / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_p1 / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_sid / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_kk / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_h1 / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_h2 / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_p4 / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_c / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_mu/ $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_telat / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_pulang / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_keluar / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_ringan / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_lanjut / $d_hk2) * 100), 2) ?> %</td>
                    <td><?php echo round((($c_berat / $d_hk2) * 100), 2) ?> %</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    $(document).ready(function() {
      ...
      var table = $('#rekap_absen').DataTable({
        ...
        buttons: [{
          ...
          title: 'Laporan Absensi Mingguan Periode ' + mulai + ' s/d ' + sampai,
        }]
      });
    });
  </script>
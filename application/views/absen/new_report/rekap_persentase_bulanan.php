        ...
        foreach ($bagian->result() as $b) {
          ...
          if($karyawan->num_rows() == 0)
          {
            ...
            foreach ($karyawan->result() as $k) { 
              if($k->recid_bag == '24')
              {
                $hitung_hk = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan join hris.bagian b on b.recid_bag = k.recid_bag where b.recid_bag = 24 and k.recid_karyawan = $k->recid_karyawan and month(h.tanggal) = $bulan  and year(h.tanggal) = $tahun and status != 24;");
              ...
              $m = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun  and h.status = '11' and k.recid_karyawan = $k->recid_karyawan");
              
              $p1 = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '5' and k.recid_karyawan = $k->recid_karyawan");
              
              $sid = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '2' and k.recid_karyawan = $k->recid_karyawan");
              
              $kk = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '25' and k.recid_karyawan = $k->recid_karyawan");
              
              $h1 = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '6' and k.recid_karyawan = $k->recid_karyawan");
              
              $h2 = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '7' and k.recid_karyawan = $k->recid_karyawan");
              
              $p4 = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '12' and k.recid_karyawan = $k->recid_karyawan");
              
              $c = $db2->query("SELECT * FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and h.status = '3' and k.recid_karyawan = $k->recid_karyawan");
              
              
              $fnorma = $db2->query("SELECT count(h.recid_absen) as jml_norma FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and $norma and k.recid_karyawan = $k->recid_karyawan");
              
              foreach ($fnorma->result() as $fnorma) {
                $jml_norma = $fnorma->jml_norma;
              } 
              
              $fnnorma = $db2->query("SELECT count(h.recid_absen) as jml_nonnorma FROM hadir_hris h join hris.karyawan k on k.recid_karyawan = h.recid_karyawan where (month(tanggal) = $bulan) and year(tanggal) = $tahun and $non_norma and k.recid_karyawan = $k->recid_karyawan");
              
              foreach ($fnnorma->result() as $fnnorma) {
                $jml_nonnorma = $fnnorma->jml_nonnorma;
              } 
              
              $telat = $db2->query("SELECT * from izin where month(tgl_izin) = $bulan and year(tgl_izin) = $tahun and jenis = 'Terlambat' and recid_karyawan = $k->recid_karyawan and is_delete = '0'");
              ...
              $ringan = $db2->query("SELECT * from absensi_hris where month(tanggal) = $bulan and year(tanggal) = $tahun and jenis_absen = 2 and kategori = 'Ringan' and recid_karyawan = $k->recid_karyawan");
              
              $lanjut =  $db2->query("SELECT * from absensi_hris where month(tanggal) = $bulan and year(tanggal) = $tahun and jenis_absen = 2 and kategori = 'Berkelanjutan' and recid_karyawan = $k->recid_karyawan");
              
              $berat =  $db2->query("SELECT * from absensi_hris where month(tanggal) = $bulan and year(tanggal) = $tahun and jenis_absen = 2 and kategori = 'Berat' and recid_karyawan = $k->recid_karyawan");

      ...
      <div class="right_col" role="main">
        ...
        <?php
        if($role == '1' or $role == '2'  or $role == '25'){ ?>
         <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
    ...
    <?php } ?>
     <?php
         if($role == '1' or $role == '2' or $role == '5' or $role == '24' or $role == '25' or $role == '26' or $role == '27' or $role == '28' or $role == '29' ){ ?>
          <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
...
<?php
if($role == '1' or $role == '3' or $role == '5' or $role == '24' or $role == '25'){ ?>
  <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
...
<?php
if($role == '1' or $role == '4' or $role == '25'){ ?>
  <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
...
if($role == '1' or $role == '3' or $role == '5' or $role == '24' or $role == '25'){ ?>
  ...
  <div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        <div class="x_title">
          <h2>Grafik Persentase Kehadiran dengan Tidak Termasuk Kategori Cuti</h2>
          <div class="clearfix"></div>
        </div>
        <div class="x_content">
...
if($role == '1' or $role == '2' or $role == '3'  or $role == '5' or $role == '24' or $role == '25'){ ?>
  ...
  <div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        <div class="x_title">
          <h2>Grafik Persentase Kehadiran Termasuk  Kategori Cuti</h2>
          <div class="clearfix"></div>
        </div>
        <div class="x_content">
...
if($role == '1' or $role == '2' or $role == '3'  or $role == '5' or $role == '24' or $role == '25'){ ?>
  ...
  <?php } ?>
   ...
   <?php
   if($role == '1' or $role == '2' or $role == '4' or $role == '24' or $role == '25'){ ?>
    <!-- Notif -->
    <?php if($role == '4'){ ?>
      <div class="row">
      <?php } ?>
...
<?php
if($role == '1' or $role == '2' or $role == '5' or $role == '27' or $role == '29'){ ?>
  <!-- Notif Expedisi-->
  <div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        <div class="x_title">
          <h2>Notifikasi Expedisi HC</h2>
          <div class="clearfix"></div>
        </div>
        <div class="x_content">
 ...
 var dd = today.getDate();
      ...
      <script type="text/javascript">
        ...
        option = {
          ...
          legend: {
            data:['Produksi','Marketing', 'Admin', 'Bis-Dev', 'Lain-Lain']
          },
          toolbox: {
            show : true,
            feature : {
          ...
          xAxis : [
          {
            ...
            $data_x = array();
            $tahun = date('Y');
            $bulan = $this->db->query("SELECT distinct(bulan) from closing_karyawan where tahun = $tahun order by bulan asc");
            foreach ($bulan->result() as $bln) { 
              $moon = $bln->bulan; 
              array_push($data_x, $moon);
            }
            ?>
            data : [<?php 
                    for($i=0;$i<count($data_x); $i++)
                    {
                      $month_name = date("F", mktime(0, 0, 0, $data_x[$i], 10));
                      echo "'$month_name',"; 
                    }
            ?>],
          }
          ...
          series : [
          {
            ...
            <?php
            $tahun = date('Y');
            $query = $this->db->query("SELECT bulan, sum(hk) as total_hk FROM closing_karyawan where tahun = '$tahun' and department = 'Produksi' Group by bulan");
            ?>
            data:[
              <?php
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and CODE != 'T' and department='Produksi'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
                echo "'$persen', ";
              }
              ?>
            ],
          },
          {
            ...
            smooth: true,
             <?php
            $tahun = date('Y');
            $query = $this->db->query("SELECT bulan, sum(hk) as total_hk FROM closing_karyawan where tahun = '$tahun' and department = 'Marketing' Group by bulan");
            ?>
            data:[
              <?php
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and CODE != 'T' and department='Marketing'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
               $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
          {
            ...
            smooth: true,
             <?php
            $tahun = date('Y');
            $query = $this->db->query("SELECT bulan, sum(hk) as total_hk FROM closing_karyawan where tahun = '$tahun' and department = 'Keu & Adm' Group by bulan");
            ?>
            data:[
              <?php
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and CODE != 'T' and department='Keu & Adm'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
           {
            ...
            smooth: true,
             <?php
            $tahun = date('Y');
            $query = $this->db->query("SELECT bulan, sum(hk) as total_hk FROM closing_karyawan where tahun = '$tahun' and department = 'BUSINESS & DEV' Group by bulan");
            ?>
            data:[
              <?php
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and CODE != 'T' and department='BUSINESS & DEV'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
           {
            ...
            smooth: true,
             <?php
            $tahun = date('Y');
            $query = $this->db->query("SELECT bulan, sum(hk) as total_hk FROM closing_karyawan where tahun = '$tahun' and department = 'Lain-lain' Group by bulan");
            ?>
            data:[
              <?php
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and CODE != 'T' and department='Lain-lain'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
        ...
        option = {
          ...
          series : [
          {
            ...
            data:[
              ...
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and department='Produksi'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
          {
            ...
            data:[
              ...
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and department='Marketing'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
               $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
          {
            ...
            data:[
              ...
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and department='Keu & Adm'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
           {
            ...
            data:[
              ...
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and department='BUSINESS & DEV'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
          ...
          },
           {
            ...
            data:[
              ...
              foreach ($query->result() as $prd) {
                $tot_hk = $prd->total_hk;
                $query_b = $this->db->query("SELECT  count(m.nik) as jml from karyawan k join master_absen.mangkir m on k.nik = m.nik join bagian b on k.recid_bag = b.recid_bag where (month(TANGGAL) = '$prd->bulan' and YEAR(TANGGAL) = '$tahun') and department='Lain-lain'");
                foreach ($query_b->result() as $abssen) {
                  $tot_absen = $abssen->jml;                  
                }
                $persen = round((($tot_hk - $tot_absen) / $tot_hk ) * 100);
        ...
        option = {
          ...
          xAxis : [
          {
           ...
           <?php 
            $data_x = array();
            $tahun = date('Y');
            $bulan = $this->db->query("SELECT distinct(month(TANGGAL)) as bulan from master_absen.mangkir where YEAR(TANGGAL) = $tahun order by month(TANGGAL) asc");
            foreach ($bulan->result() as $bln) { 
              $moon = $bln->bulan; 
              array_push($data_x, $moon);
            }
          ...
          series : [
          {
            ...
            for($i = 1; $i<=12; $i++)
            {
              $query = $this->db->query("SELECT count(m.nik) as cuti from master_absen.mangkir m join hris.karyawan k on k.nik = m.nik join hris.bagian b on b.recid_bag = k.recid_bag where YEAR(TANGGAL) = '$tahun' and CODE = 'T' and month(TANGGAL) = '$i'");
              foreach ($query->result() as $cuti) {
                echo "'$cuti->cuti', ";
              }
            }
              ?>
          ...
          },
          {
            ...
            for($i = 1; $i<=12; $i++)
            {
              $query = $this->db->query("SELECT count(m.nik) as sid from master_absen.mangkir m join hris.karyawan k on k.nik = m.nik join hris.bagian b on b.recid_bag = k.recid_bag where YEAR(TANGGAL) = '$tahun' and CODE = 'S1D' and month(TANGGAL) = '$i'");
              foreach ($query->result() as $sid) {
                echo "'$sid->sid', ";
              }
            }
              ?>
          ...
          },
           {
            ...
            for($i = 1; $i<=12; $i++)
            {
              $query = $this->db->query("SELECT count(m.nik) as p1 from master_absen.mangkir m join hris.karyawan k on k.nik = m.nik join hris.bagian b on b.recid_bag = k.recid_bag where YEAR(TANGGAL) = '$tahun' and CODE = 'P1' and month(TANGGAL) = '$i'");
          ...
          },
           {
            ...
            for($i = 1; $i<=12; $i++)
            {
              $query = $this->db->query("SELECT count(m.nik) as ma from master_absen.mangkir m join hris.karyawan k on k.nik = m.nik join hris.bagian b on b.recid_bag = k.recid_bag where YEAR(TANGGAL) = '$tahun' and CODE = 'MA' and month(TANGGAL) = '$i'");

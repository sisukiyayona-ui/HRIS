                  ...
                  <!-- Total Per Tahun end -->
                   ...
                   <?php 
                      for($x=0; $x<15;$x++){
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                           $moon = substr($month[$x], 8);
                           $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and kuartal = '$moon'")->result();
                         $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and kuartal = '$moon'")->result();
                        ...
                        <?php }else{
                         $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and bulan = '$month[$x]'")->result();
                         $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and bulan = '$month[$x]'")->result();
                         foreach ($total_budget as $total_budget) { }
                           if($total_budget->total_budget == null){
                            $total_budget = '0';
                          }else{
             ...
             <!-- GRAND TOTAL -->
                ...
                <?php 
                      for($x=0; $x<15;$x++){
                        $moon = substr($month[$x], 8);
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                          $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb  where tahun = YEAR(CURDATE()) and kuartal = '$moon'")->result();
                          $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl and kuartal = '$moon'")->result();
                        ...
                        <?php }else{
                          $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb  where tahun = YEAR(CURDATE()) and bulan = '$month[$x]'")->result();
                          $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl and bulan = '$month[$x]'")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      <!-- Total kuartal IV -->
                      <?php 
                       $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb  where tahun = YEAR(CURDATE()) and kuartal = 'IV'")->result();
                          $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl and kuartal = 'IV'")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{

              ...
              foreach ($pay_group as $pay) {?>
                ...
                foreach ($bagian as $dbagian) { ?>
                  ...
                  <tr>
                    ...
                    <?php 
                    $budget = $this->db->query("SELECT * from master_budget mb join bagian b on b.recid_bag = mb.recid_bag where tahun  = YEAR(CURDATE()) and b.recid_bag = '$dbagian->recid_bag'")->result();
                    foreach ($budget as $dbudget) { 
                      $mbudget = round($dbudget->jml_jam);
                      ?>
                      ...
                      if($mbudget == 0){
                        ...
                        $selisih = round(($total / $mbudget) * 100);
                      }
                    ...
                    foreach ($tahun_budget as $tbudget) {}
                      ...
                      foreach ($tahun_realisasi as $trealisasi) {}
                        if($trealisasi->tahun_realisasi == 0){
                          ...
                          $trealisasi = round($trealisasi->tahun_realisasi);
                        }
                        if($tbudget == '0'){
                          $tahun_capai = '0';
                        }else{
                     ...
                     <tr style="background-color: #b5dce1; color: #000"><td><b>Jumlah <?php echo $pay->pay_group ?></b></td>
                        ...
                        for ($i=0;$i < count($month); $i++) {
                          $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and bulan = '$month[$i]' and ($role)")->result();
                          $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and bulan = '$month[$i]' and ($role)")->result();
                          foreach ($total_budget as $total_budget) { }
                            if($total_budget->total_budget == null){
                                $total_budget = '0';
                              }else{
                                $total_budget = round($total_budget->total_budget);
                              }
                            foreach ($total_realisasi as $total_realisasi) { }
                              if($total_realisasi->total_realisasi == null){
                                $total_realisasi = '0';
                              }else{
                                $total_realisasi = round($total_realisasi->total_realisasi);
                              }
                              if($total_budget == 0){
                                $total_pencapaian = '0';
                              }else{
                  ...
                  foreach ($realisasi_total as $rtotal) {}
                    if($rtotal->realisasi_total == null){
                      ...
                      $rtotal = round($rtotal->realisasi_total);
                    }
                    if($btotal == '0'){
                      $total_capai = '0';
                    }else{
                ...
                <!-- GRAND TOTAL -->
                <?php 
                 $cek = $this->db->query("SELECT DISTINCT pay_group from bagian b join master_budget mb on b.recid_bag = mb.recid_bag where mb.tahun =  YEAR(CURDATE()) and $role ");
                    if ($cek->num_rows() > 0) { ?>
                <tr style="background-color:#4d648d; color: #fff";>
                  <td><b>Grand Total</b></td>
                  <?php
                  for ($i=0;$i < count($month); $i++) {
                   $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and bulan = '$month[$i]' and ($role)")->result();
                   $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where bulan = '$month[$i]' and ($role)")->result();

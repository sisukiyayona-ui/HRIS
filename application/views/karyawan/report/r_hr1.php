        ...
        <div class="x_panel">
          ...
          <div class="x_content">
            ...
            <table id="tr_hr" class="table table-striped table-bordered" width="100%">
              ...
              //jml BOD
                    $pres = $this->db->query("SELECT count(recid_karyawan) as pres FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and sts_aktif = 'Aktif'")->result();
                    ...
                    <tr style="background-color: #CCC"><td><b><?php echo strtoupper($dept->department) ?></b></td>
                      ...
                      <?php
                      $tetap_pres = $this->db->query("SELECT count(recid_karyawan) as tetap FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and sts_jbtn = 'Tetap' and sts_aktif = 'Aktif'")->result();
                      foreach ($tetap_pres as $tetap_pres) { $tetap_pres = $tetap_pres->tetap;}

                      $kontrak_pres = $this->db->query("SELECT count(recid_karyawan) as kontrak FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where  j.recid_jbtn = '1' and sts_jbtn = 'Kontrak' and sts_aktif = 'Aktif'")->result();
                      foreach ($kontrak_pres as $kontrak_pres) {$kontrak_pres = $kontrak_pres->kontrak;}

                      $honor_pres = $this->db->query("SELECT count(recid_karyawan) as honor FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and sts_jbtn = 'Honorer' and sts_aktif = 'Aktif'")->result();
                      ...
                      <td><center><?php  echo $tetap_pres ?></td>
                        <td><center><?php  echo $kontrak_pres ?></td>
                          <td><center><?php  echo $honor_pres?></td>
                           <td><center><b><?php echo $pres ?></b></center></td>
                       <?php
                       $pria_pres = $this->db->query("SELECT count(recid_karyawan) as pria_pres FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and jenkel = 'Laki - laki' and sts_aktif = 'Aktif'")->result();
                       foreach ($pria_pres as $pria_pres) {$pria_pres = $pria_pres->pria_pres;}
                       $Perempuan_pres = $this->db->query("SELECT count(recid_karyawan) as Perempuan_pres FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and jenkel = 'Perempuan' and sts_aktif = 'Aktif'")->result();
                       foreach ($Perempuan_pres as $Perempuan_pres) {$Perempuan_pres = $Perempuan_pres->Perempuan_pres;}
                       ?>
                       <td><center><?php  echo $Perempuan_pres ?></center></td>
                      ...
                      <?php } ?>
                      <?php 
                      $d3_pres = $this->db->query("SELECT count(recid_karyawan) as d3_pres FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and pendidikan = 'D3' and sts_aktif = 'Aktif'")->result();
                      foreach ($d3_pres as $d3_pres) {$d3_pres = $d3_pres->d3_pres;}
                      $s1_pres = $this->db->query("SELECT count(recid_karyawan) as s1_pres FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1'and pendidikan = 'S1' and sts_aktif = 'Aktif'")->result();
                      foreach ($s1_pres as $s1_pres) {$s1_pres = $s1_pres->s1_pres;}
                      $s2_pres = $this->db->query("SELECT count(recid_karyawan) as s2_pres FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where j.recid_jbtn = '1' and pendidikan = 'S2' and sts_aktif = 'Aktif'")->result();
                 ...
                 <!--  <td><center><?php  echo $tetap_top->tetap ?></center></td>
                  ...
                  <td><center><b><?php echo $bod+$gm; ?></b></center></td>
                  <?php
                    $pria_bod = $this->db->query("SELECT count(recid_karyawan) as pria_bod FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where (j.recid_jbtn = '10' or j.recid_jbtn = '11') and b.department = '$dept->department' and jenkel = 'Laki - laki' and sts_aktif = 'Aktif'")->result();
                     foreach ($pria_bod as $pria_bod) {$pria_bod = $pria_bod->pria_bod;
                      $L1a = $L1a + $pria_bod;}
                      $Perempuan_bod = $this->db->query("SELECT count(recid_karyawan) as Perempuan_bod FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur join jabatan j on k.recid_jbtn = j.recid_jbtn where (j.recid_jbtn = '10' or j.recid_jbtn = '11') and b.department = '$dept->department' and jenkel = 'Perempuan' and sts_aktif = 'Aktif'")->result();
                     foreach ($Perempuan_bod as $Perempuan_bod) {$Perempuan_bod = $Perempuan_bod->Perempuan_bod;
                      $P1a = $P1a + $Perempuan_bod;}
                  ?>
                  <td><center><?php  echo $Perempuan_bod ?></center></td>
                      ...
                      <!-- DATA PER KOLOM -->
                       ...
                       // }
                         ...
                         $jml2a = $jml2a + $jml2;
                                ...
                                //jml Pria
                         $L = $this->db->query("SELECT s.nama_struktur, count(recid_karyawan) as total_l FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur where k.jenkel = 'Laki - laki' and s.nama_struktur = '$bagian->nama_struktur'  and sts_aktif = 'Aktif'")->result();
                         foreach ($L as $pria) {
                           $L = $pria->total_l;
                           $L1 = $L1 + $L;
                         }
                                 //jml_Perempuan
                         $P = $this->db->query("SELECT count(recid_karyawan) as total_p FROM karyawan k join bagian b on k.recid_bag = b.recid_bag join struktur s on s.recid_struktur = b.recid_struktur where k.jenkel = 'Perempuan' and s.nama_struktur = '$bagian->nama_struktur' and sts_aktif = 'Aktif'")->result();
                         foreach ($P as $Perempuan) {
                           $P = $Perempuan->total_p;
                           $P1 = $P1 + $P;
                         }
            ...
            <tr style="background-color:#4d648d; color: #fff">
             ...
             <td><center><b><?php echo $bod1; ?></b></td>
               <td><center><b><?php echo $gm1; ?></b></td>
                 ...
                 <td><center><b><?php echo $P1+$P1a+$Perempuan_pres?></b></center></td>

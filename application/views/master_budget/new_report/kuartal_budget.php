          ...
          <div class="x_content">
            ...
            <tbody>
             ...
             foreach ($pay_group as $pay) {?>
              ...
              <?php 
               $bagian = $this->db->query("SELECT b.nama_bag, indeks_hr, mbl.recid_bag from stkl s right join master_budget mbl on s.recid_mbl = mbl.recid_mbl join bagian b on b.recid_bag = mbl.recid_bag  where pay_group = '$pay->pay_group' and mbl.tahun = '$tahun' group by nama_bag order by indeks_hr asc")->result();
                      ...
                      for($x=0; $x<15;$x++){
                        ...
                        <?php }else{ 
                           ...
                           foreach ($budget as $dbudget) { 
                            ...
                            $real = $this->db->query("SELECT sum(totjam_real) as total from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl where s.recid_mbl = '$dbudget->recid_mbl'")->result();
                   ...
                   <?php 
                      for($x=0; $x<15;$x++){
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                         ...
                         $total_realisasi = $this->db->query("SELECT sum(totjam_real) as total_realisasi from stkl s join master_budget mb on s.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and kuartal = '$moon' and mb.tahun =  '$tahun' ")->result();
                        ...
                        <?php }else{
                         ...
                         $total_realisasi = $this->db->query("SELECT sum(totjam_real) as total_realisasi from stkl s join master_budget mb on s.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and bulan = '$month[$x]'")->result();
                         foreach ($total_budget as $total_budget) { }
                           if($total_budget->total_budget == null){
                            $total_budget = '0';
                          }else{
                      ...
                      $realisasi_total = $this->db->query("SELECT sum(totjam_real) as realisasi_total from stkl s  join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group'")->result();
                ...
                <?php 
                      for($x=0; $x<15;$x++){
                        ...
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                          ...
                          $grandr = $this->db->query("SELECT sum(totjam_real) as grandr  from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl and kuartal = '$moon' and mb.tahun =  '$tahun' ")->result();
                        ...
                        <?php }else{
                          ...
                          $grandr = $this->db->query("SELECT sum(totjam_real) as grandr  from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl and bulan = '$month[$x]' and mb.tahun =  '$tahun'")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      <?php 
                       $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb  where tahun = '$tahun' and kuartal = 'IV'")->result();
                          $grandr = $this->db->query("SELECT sum(totjam_real) as grandr  from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl and kuartal = 'IV' and mb.tahun =  '$tahun'")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      foreach ($grand_budget as $gbudget) { }
                        ...
                        $grand_real = $this->db->query("SELECT sum(totjam_real) as greal from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl where mb.tahun =  '$tahun'")->result();

      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            ...
            <h2><a href="<?php echo base_url()?>Karyawan/dash"><i class="fa fa-arrow-circle-o-left"></i></a> | Report Master 
            <?php echo $tahun;
            ?></h2>
          ...
          <div class="x_content">
            ...
            <tbody>
             <?php  
             $pay_group = $this->db->query("SELECT DISTINCT pay_group from bagian b join master_budget mb on b.recid_bag = mb.recid_bag join department d on d.recid_department = b.recid_department where mb.tahun =  '$tahun' and $role")->result();
             foreach ($pay_group as $pay) {?>
              ...
              <?php 
               $bagian = $this->db->query("SELECT b.nama_bag, indeks_hr, mbl.recid_bag from stkl s right join master_budget mbl on s.recid_mbl = mbl.recid_mbl join bagian b on b.recid_bag = mbl.recid_bag where pay_group = '$pay->pay_group' and mbl.tahun = '$tahun' and ($role) group by nama_bag order by indeks_hr asc")->result();
                foreach ($bagian as $dbagian) { ?>
                    ...
                    <?php 
                      for($x=0; $x<15;$x++){
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                            ...
                            $real_k = $this->db->query("SELECT sum(totjam_real) as total from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl where mb.recid_bag = '$dbagian->recid_bag' and mb.kuartal = '$moon' and mb.tahun =  '$tahun'")->result();
                        ...
                        <?php }else{ 
                           ...
                           foreach ($budget as $dbudget) { 
                            ...
                            $real = $this->db->query("SELECT sum(totjam_real) as total from stkl s  where s.recid_mbl = '$dbudget->recid_mbl'")->result();
                  ...
                  $real_4 = $this->db->query("SELECT sum(totjam_real) as total from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl where mb.recid_bag = '$dbagian->recid_bag' and mb.kuartal = 'IV'")->result();
                  ...
                  ?>
                  <td style="background-color: #d4e7bf; color: #000"><center><?php if($selisih4 > 100 or ($mbudget4 == '0' and $total > '0')){echo "<font color= 'red'><b>$mbudget4</b>";}else{ echo $mbudget4;} ?></center></td>
                  <td style="background-color: #abe0ee; color: #000"><center><?php if($selisih4 > 100  or ($mbudget4 == '0' and $total > '0')){echo "<font color= 'red'><b>$total</b>";}else{ echo $total;} ?></center></td>
                  ...
                  $tahun_realisasi = $this->db->query("SELECT sum(totjam_real) as tahun_realisasi from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and mb.recid_bag = '$dbagian->recid_bag'")->result();
                  ...
                  <!-- Total Per Tahun end -->
                   ...
                   <?php 
                      for($x=0; $x<15;$x++){
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                           $moon = substr($month[$x], 8);
                           $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and kuartal = '$moon' and ($role)")->result();
                         $total_realisasi = $this->db->query("SELECT sum(totjam_real) as total_realisasi from stkl s join master_budget mb on s.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and kuartal = '$moon' and ($role)")->result();
                        ...
                        <?php }else{
                         $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and bulan = '$month[$x]' and ($role)")->result();
                         $total_realisasi = $this->db->query("SELECT sum(totjam_real) as total_realisasi from stkl s join master_budget mb on s.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and bulan = '$month[$x]' and ($role)")->result();
                         foreach ($total_budget as $total_budget) { }
                           if($total_budget->total_budget == null){
                            $total_budget = '0';
                          }else{
                      ...
                      <?php
                        $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and kuartal = 'IV'")->result();
                         $total_realisasi = $this->db->query("SELECT sum(totjam_real) as total_realisasi from stkl s join master_budget mb on s.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and kuartal = 'IV'")->result();
                         foreach ($total_budget as $total_budget) { }
                           if($total_budget->total_budget == null){
                            $total_budget = '0';
                          }else{
                      ...
                      <?php  
                      $budget_total = $this->db->query("SELECT sum(jml_jam) as budget_total from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and ($role)")->result();
                      $realisasi_total = $this->db->query("SELECT sum(totjam_real) as realisasi_total from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and ($role)")->result();
             ...
             <!-- GRAND TOTAL -->
             <?php  $cek = $this->db->query("SELECT DISTINCT pay_group from bagian b join master_budget mb on b.recid_bag = mb.recid_bag where mb.tahun =  '$tahun' and $role ");
             if ($cek->num_rows() > 0) { ?>
                ...
                <?php 
                      for($x=0; $x<15;$x++){
                        $moon = substr($month[$x], 8);
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                          $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and kuartal = '$moon' and ($role)")->result();
                          $grandr = $this->db->query("SELECT sum(totjam_real) as grandr  from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where kuartal = '$moon' and ($role) and tahun = '$tahun'")->result();
                        ...
                        <?php }else{
                          $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and bulan = '$month[$x]' and ($role)")->result();
                          $grandr = $this->db->query("SELECT sum(totjam_real) as grandr  from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where bulan = '$month[$x]' and ($role) and tahun = '$tahun'")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      <!-- Total kuartal IV -->
                      <?php 
                       $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and kuartal = 'IV' and ($role)")->result();
                          $grandr = $this->db->query("SELECT sum(totjam_real) as grandr  from stkl s  join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where kuartal = 'IV' and ($role)")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      <!-- Total Paygroup Pertahun -->
                      <?php 
                      $grand_budget = $this->db->query("SELECT sum(jml_jam) as gbudget  from master_budget mb join bagian b on b.recid_bag = mb.recid_bag  where tahun = '$tahun' and ($role)")->result();
                      foreach ($grand_budget as $gbudget) { }
                        ...
                        $grand_real = $this->db->query("SELECT sum(totjam_real) as greal from stkl s join master_budget mb on mb.recid_mbl = s.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where ($role) and mb.tahun =  '$tahun'")->result();

          ...
          <div class="x_content">
            ...
            <tbody>
              ...
              foreach ($pay_group as $pay) {?>
                    ...
                    foreach ($tahun_budget as $tbudget) {}
                      ...
                      <tr style="background-color: #b5dce1; color: #000"><td><b>Jumlah <?php echo $pay->pay_group ?></b></td>
                        ...
                        for ($i=0;$i < count($month); $i++) {
                          $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and bulan = '$month[$i]'")->result();
                          $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = '$tahun' and pay_group = '$pay->pay_group' and bulan = '$month[$i]'")->result();
                ...
                <!-- GRAND TOTAL -->
                <?php 
                 $cek = $this->db->query("SELECT DISTINCT pay_group from bagian b join master_budget mb on b.recid_bag = mb.recid_bag where mb.tahun =  '$tahun' order by b.pay_group asc");
                    if ($cek->num_rows() > 0) { ?>
                <tr style="background-color:#4d648d; color: #fff";>
                  <td><b>Grand Total</b></td>
                  <?php
                  for ($i=0;$i < count($month); $i++) {
                   $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb  where tahun = '$tahun'()) and bulan = '$month[$i]'")->result();
                   $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl and bulan = '$month[$i]' and tahun = '$tahun'")->result();
                   foreach ($grandb as $grandb) { }
                    ...
                    $grand_budget = $this->db->query("SELECT sum(jml_jam) as gbudget  from master_budget mb  where tahun = YEAR'$tahun'")->result();

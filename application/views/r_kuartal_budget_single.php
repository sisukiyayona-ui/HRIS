  ...
  <div class="">
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2><a href="<?php echo base_url()?>Karyawan/dash"><i class="fa fa-arrow-circle-o-left"></i></a> | Report Master Budget Kuartal</h2>
          ...
          <div class="x_content">
            ...
            <tbody>
             <?php  
             $pay_group = $this->db->query("SELECT DISTINCT pay_group from bagian b join master_budget mb on b.recid_bag = mb.recid_bag where mb.tahun =  YEAR(CURDATE()) and $role")->result();
             foreach ($pay_group as $pay) {?>
              ...
              <?php 
               $bagian = $this->db->query("SELECT b.nama_bag, mbl.recid_bag from pengajuan_lembur pl right join master_budget mbl on pl.recid_mbl = mbl.recid_mbl join bagian b on b.recid_bag = mbl.recid_bag left join realisasi_lembur rl on rl.recid_plembur = pl.recid_plembur where pay_group = '$pay->pay_group'  and ($role) group by nama_bag")->result();
                foreach ($bagian as $dbagian) { ?>
                  <tr><td><?php echo $dbagian->nama_bag?></td>
                    <!-- kelipatan 4 -->
                    <?php 
                      for($x=0; $x<15;$x++){
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                            $moon = substr($month[$x], 8);
                            $budget_k = $this->db->query("SELECT sum(jml_jam) as jml_jam from master_budget where tahun  = YEAR(CURDATE()) and recid_bag = '$dbagian->recid_bag' and kuartal = '$moon'")->result(); 
                            foreach ($budget_k as $dbudgetk) { 
                              $mbudgetk = $dbudgetk->jml_jam;
                              $mbudgetk = round($mbudgetk,2);
                            }
                            $real_k = $this->db->query("SELECT sum(realisasi_jam) as total from realisasi_lembur rl join pengajuan_lembur pl on rl.recid_plembur = pl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl where mb.recid_bag = '$dbagian->recid_bag' and mb.kuartal = '$moon'")->result();
                        ...
                        <?php }else{ 
                           $budget = $this->db->query("SELECT * from master_budget where tahun  = YEAR(CURDATE()) and recid_bag = '$dbagian->recid_bag' and bulan = '$month[$x]'")->result();
                   ...
                   <!-- Kuartal IV -->
                   <?php
                   $budget_4 = $this->db->query("SELECT sum(jml_jam) as jml_jam from master_budget where tahun  = YEAR(CURDATE()) and recid_bag = '$dbagian->recid_bag' and kuartal = 'IV'")->result(); 
                  ...
                  <!-- Total Per Tahun end -->
                   ...
                   <?php 
                      for($x=0; $x<15;$x++){
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                           $moon = substr($month[$x], 8);
                           $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and kuartal = '$moon' and ($role)")->result();
                         $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and kuartal = '$moon' and ($role)")->result();
                        ...
                        <?php }else{
                         $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and bulan = '$month[$x]' and ($role)")->result();
                         $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and bulan = '$month[$x]' and ($role)")->result();
                         foreach ($total_budget as $total_budget) { }
                           if($total_budget->total_budget == null){
                            $total_budget = '0';
                          }else{
                      ...
                      <!-- Total kuartal IV -->
                      <?php
                        $total_budget = $this->db->query("SELECT sum(jml_jam) as total_budget from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and kuartal = 'IV'")->result();
                         $total_realisasi = $this->db->query("SELECT sum(realisasi_jam) as total_realisasi from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on pl.recid_mbl = mb.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and kuartal = 'IV'")->result();
                         foreach ($total_budget as $total_budget) { }
                           if($total_budget->total_budget == null){
                            $total_budget = '0';
                          }else{
                      ...
                      <?php  
                      $budget_total = $this->db->query("SELECT sum(jml_jam) as budget_total from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and ($role)")->result();
                      $realisasi_total = $this->db->query("SELECT sum(realisasi_jam) as realisasi_total from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and pay_group = '$pay->pay_group' and ($role)")->result();
             ...
             <!-- GRAND TOTAL -->
             <?php  $cek = $this->db->query("SELECT DISTINCT pay_group from bagian b join master_budget mb on b.recid_bag = mb.recid_bag where mb.tahun =  YEAR(CURDATE()) and $role ");
             if ($cek->num_rows() > 0) { ?>
                ...
                <?php 
                      for($x=0; $x<15;$x++){
                        $moon = substr($month[$x], 8);
                        if((($x+4) == 3 || ($x+4) == 7 || ($x+4) == 11 || ($x+4) == 15 || ($x+4) == 19  ) and $x != 0){ 
                          $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and kuartal = '$moon' and ($role)")->result();
                          $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where kuartal = '$moon' and ($role)")->result();
                        ...
                        <?php }else{
                          $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and bulan = '$month[$x]' and ($role)")->result();
                          $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where bulan = '$month[$x]' and ($role)")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      <!-- Total kuartal IV -->
                      <?php 
                       $grandb = $this->db->query("SELECT sum(jml_jam) as grandb  from master_budget mb join bagian b on mb.recid_bag = b.recid_bag where tahun = YEAR(CURDATE()) and kuartal = 'IV' and ($role)")->result();
                          $grandr = $this->db->query("SELECT sum(realisasi_jam) as grandr  from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where kuartal = 'IV' and ($role)")->result();
                          foreach ($grandb as $grandb) { }
                            if($grandb->grandb == '0' || $grandb->grandb == null){
                              $grandb = '0';
                            }else{
                      ...
                      <!-- Total Paygroup Pertahun -->
                      <?php 
                      $grand_budget = $this->db->query("SELECT sum(jml_jam) as gbudget  from master_budget mb join bagian b on b.recid_bag = mb.recid_bag  where tahun = YEAR(CURDATE()) and ($role)")->result();
                      foreach ($grand_budget as $gbudget) { }
                        ...
                        $grand_real = $this->db->query("SELECT sum(realisasi_jam) as greal from realisasi_lembur rl join pengajuan_lembur pl on pl.recid_plembur = rl.recid_plembur join master_budget mb on mb.recid_mbl = pl.recid_mbl join bagian b on mb.recid_bag = b.recid_bag where ($role)")->result();
